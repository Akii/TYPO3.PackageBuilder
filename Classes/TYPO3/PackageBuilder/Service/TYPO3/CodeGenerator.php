...
class CodeGenerator extends \TYPO3\PackageBuilder\Service\AbstractCodeGenerator {
	...
	public function build(Model\PackageInterface $extension) {
		...
		if (!is_dir($this->extensionDirectory)) {
			\TYPO3\Flow\Utility\Files::createDirectoryRecursively($this->extensionDirectory);
		...
		\TYPO3\Flow\Utility\Files::createDirectoryRecursively($this->configurationDirectory);

		\TYPO3\Flow\Utility\Files::createDirectoryRecursively($this->privateResourcesDirectory);
	...
	protected function generateYamlSettingsFile() {
		...
		if (!file_exists($this->configurationDirectory . 'ExtensionBuilder/settings.yaml')) {
			\TYPO3\Flow\Utility\Files::createDirectoryRecursively($this->configurationDirectory . 'ExtensionBuilder');
	...
	protected function generateTCAFiles() {
		// Generate TCA
		try {
			\TYPO3\Flow\Utility\Files::createDirectoryRecursively($this->extensionDirectory, 'Configuration/TCA');
	...
	protected function generateLocallangFiles() {
		...
		try {
			...
			\TYPO3\Flow\Utility\Files::createDirectoryRecursively($this->languageDirectory);
	...
	protected function generateDocumentationFiles() {
		...
		$docFiles = array();
		$docFiles = \TYPO3\Flow\Utility\Files::readDirectoryRecursively($documentationDirectory, NULL, TRUE, $docFiles);
