...
class PackageFactory extends \TYPO3\PackageBuilder\Service\AbstractPackageFactory {
	...
	protected function setExtensionProperties(&$extension, $propertyConfiguration) {
		...
		if (!empty($propertyConfiguration['emConf']['dependsOn'])) {
			$dependencies = array();
			$lines = \TYPO3\Flow\Utility\Arrays::trimExplode('
', $propertyConfiguration['emConf']['dependsOn']);
			foreach ($lines as $line) {
				if (strpos($line, '=>')) {
					list($extensionKey, $version) = \TYPO3\Flow\Utility\Arrays::trimExplode('=>', $line);
...
', $pluginValues['actions']['controllerActionCombinations'], TRUE);
			foreach ($lines as $line) {
				list($controllerName, $actionNames) = \TYPO3\Flow\Utility\Arrays::trimExplode('=>', $line);
				$controllerActionCombinations[$controllerName] = \TYPO3\Flow\Utility\Arrays::trimExplode(',', $actionNames);
			}
...
', $pluginValues['actions']['noncacheableActions'], TRUE);
			foreach ($lines as $line) {
				list($controllerName, $actionNames) = \TYPO3\Flow\Utility\Arrays::trimExplode('=>', $line);
				$noncacheableControllerActions[$controllerName] = \TYPO3\Flow\Utility\Arrays::trimExplode(',', $actionNames);
			}
...
', $pluginValues['actions']['switchableActions'], TRUE);
			...
			foreach ($lines as $line) {
				if (strpos($line, '->') === FALSE) {
					...
					$switchableAction['actions'] = \TYPO3\Flow\Utility\Arrays::trimExplode(';', $line, TRUE);
